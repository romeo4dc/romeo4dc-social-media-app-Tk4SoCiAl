...
const Explore = () => {
  ...
  //       document.querySelector('html').style.fontSize="150%!important";
  ...
  useEffect(() => {
    ...
    console.log(window.innerWidth, "sdsd")
    setLoading(true)
            ...
            return (
              <React.Fragment key={media.id + index}>
                {media.hasOwnProperty('photographer') && (
                  <div className="posts-box" onClick={createPostsCollection} key={media.id+2}>
                      <img src={media.src.large2x}  alt="Random Image" className='main-posts' data-name={media.id} username={media.photographer} image={media.src.tiny}/>    
                  </div>
                )}

                {media.hasOwnProperty('video_files') && (
                  <div className="reels-box" onClick={createPostsCollection} key={media.id}>
                    <video src={media.video_files[0].link} loop  data-name={media.id} username={media.user.name} image={media.video_pictures[0].picture}/> 
